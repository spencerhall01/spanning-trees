...
%save J.dat J -ASCII
save(['EAtrees12x12_',int2str(T)], 'J')

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Assign the magnetic fields
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%H = input('Magnetic field (element of [-2,2], glass, random): ');
H = 0;
...
elseif strcmp(H,'random') == 1
    ...
    lI = input('  Lower bound (element of [-2,2]): ');
    %check that the lower bound is possible
    while or(lI < j_range(1), lI > j_range(2))
        lI = input('   Lower bound must be an element of [-2,2]): ');
    end

    uI = input('  Upper bound (element of [lower bound,2]): ');
    %check that the upper bound is possible
    while or(uI < lI, uI > 1)
        uI = input('   Upper bound must be an element of [lower bound,2]): ');
    end
...
%save output.dat solvername properties parameters solutions energies num_occurrences timing lowest -append -ASCII
save(['10x100EAsolutions12x12_',int2str(T),int2str(R)], 'lowsol')
save EA10x100randspan12x12.dat lowest -append -ASCII
